...
var render$1 = {
  ...
  initCanvas: function initCanvas() {
    ...
    canvasData.oldLeft = canvasData.left =
      (containerData.width - canvasWidth) / 2;
    canvasData.oldTop = canvasData.top =
      (containerData.height - canvasHeight) / 2;
...
var preview$1 = {
  initPreview: function initPreview() {
    ...
    var previews = preview.querySelector
      ? [preview]
...
var events = {
  bind: function bind() {
    ...
    addListener(
      cropper,
      EVENT_POINTER_DOWN,
      (self.onCropStart = proxy(self.cropStart, self))
    );
    ...
    if (options.zoomable && options.zoomOnWheel) {
      addListener(
        cropper,
        EVENT_WHEEL,
        (self.onWheel = proxy(self.wheel, self))
      );
    }
    ...
    if (options.toggleDragModeOnDblclick) {
      addListener(
        cropper,
        EVENT_DBLCLICK,
        (self.onDblclick = proxy(self.dblclick, self))
      );
    }
    ...
    addListener(
      document,
      EVENT_POINTER_MOVE,
      (self.onCropMove = proxy(self.cropMove, self))
    );
    addListener(
      document,
      EVENT_POINTER_UP,
      (self.onCropEnd = proxy(self.cropEnd, self))
    );
    ...
    if (options.responsive) {
      addListener(
        window,
        EVENT_RESIZE,
        (self.onResize = proxy(self.resize, self))
      );
    }
  },
...
var handlers = {
  ...
  cropStart: function cropStart(event) {
    ...
    if (
      dispatchEvent(self.element, "cropstart", {
        originalEvent: e,
        action: action
      }) === false
    ) {
  ...
  cropMove: function cropMove(event) {
    ...
    if (
      dispatchEvent(self.element, "cropmove", {
        originalEvent: e,
        action: action
      }) === false
    ) {
  ...
  cropEnd: function cropEnd(event) {
    ...
    if (self.cropping) {
      ...
      toggleClass(
        self.dragBox,
        "cropper-modal",
        self.cropped && this.options.modal
      );
    }
...
var methods = {
  ...
  zoomTo: function zoomTo(ratio, _originalEvent) {
    ...
    if (ratio >= 0 && self.ready && !self.disabled && options.zoomable) {
      ...
      if (
        dispatchEvent(self.element, "zoom", {
   ...
   * @return {HTMLCanvasElement} canvas
   */
  getCroppedContext: function getCroppedContext(options) {
    var self = this;
